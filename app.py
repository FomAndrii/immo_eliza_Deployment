...
st.header(
    "Immoweb, Belgium’s top real estate site for over 20 years, has created Immoweb (:blue[ImmoEliza]) Predict, "
    "a Machine Learning Model, to forecast price of your favourite property.",
    divider=True,
)
...
living_area = st.number_input(
    "Living Area (in sqm)", min_value=10, max_value=1000, step=10
)
kitchen = st.selectbox("Fully Equipped Kitchen", ["Yes", "No"])

# Encode 'Type_of_Property' using Label Encoding
le = LabelEncoder()
property_type_encoded = le.fit_transform(
    ["HOUSE", "APARTMENT"]
)  # Fit on the categories
property_type_dict = dict(
    zip(["HOUSE", "APARTMENT"], property_type_encoded)
)  # Create a mapping
...
st.markdown(
    """:blue-background[Please, give me your feedback about our forecasting and read my Regular Column: AI in Law]"""
)
